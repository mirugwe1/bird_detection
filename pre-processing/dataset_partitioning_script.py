...
def iterate_dir(source, dest, ratio, copy_xml):
    ...
    dest = dest.replace('\\', '/')
    train_dir = os.path.join(dest, 'training_set')
    test_dir = os.path.join(dest, 'validation_set')
...
def main():
    ...
    parser.add_argument(
        '-r', '--ratio',
        help='The ratio of the number of test images over the total number of images. The default is 0.2.',
        default=0.1,
        type=float)
    parser.add_argument(
        '-x', '--xml',
